...
export async function saveOrders(
  ...
  try {
    ...
    do {
      ...
      const data = await orders.getOrders(`f_status=invoiced&per_page=100&page=${currentPage}`);

      await Promise.all(data.list.map(async (order: any) => {
        const getOrder: OrderDataProps = await orders.getOrder(order.orderId);

        const customApps: CustomApp[] = getOrder?.customData?.customApps ?? [];
        const customApp = customApps?.find((customApp) => customApp?.id === "codigovendedor");

        totalOrders++;

        console.log("🚀 ~ awaitPromise.all ~ order.orderId:", totalOrders, getOrder.orderId)
