export default class WarehouseValidation {
  ...
  validationWarehouseId() {
    ...
    this.waitForEl(".product-item").then(() => {
      this.createWarehouseMessage();
    }).catch((error) => {
      console.error("Erro ao esperar pelos elementos dinâmicos:", error);
    });
  }
  ...
  createWarehouseMessage() {
    ...
    orderForm.items.forEach((item, index) => {
      ...
      if (!logisticInfo || !logisticInfo.slas || logisticInfo.slas.length === 0) {
        console.warn(`LogisticsInfo não encontrado para o item ${item.name}.`);
        return;
      }

      const dockId = logisticInfo.slas[0]?.deliveryIds?.[0]?.dockId;
      console.log(`SKU: ${skuId}, dockId: ${dockId}, Produto: ${item.name}`);

      const productContainer = $(`.product-item[data-sku='${skuId}']`);

      if (productContainer.length > 0) {
        const shippingContainer = productContainer.find(".shipping-date");
        ...
        console.warn(`Elemento com data-sku ${skuId} não encontrado no DOM.`);
      }
    });
  }
