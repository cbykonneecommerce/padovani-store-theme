...
const FilterAnchor = (props) => {
  ...
  const getHeightContainer = () => {
    const container = document.querySelector(
      '.vtex-flex-layout-0-x-flexRow--main-section-campaign'
    )
  ...
  return (
    <Skeleton height={200}>
      <aside
        className={`${styles.FilterCustomContainer} ${
          isOpen ? styles.FilterOpen : styles.FilterClose
        } ${isAbsolute ? styles.IsAbstolute : styles.NotAbstolute}`}
      >
        {props?.img && props?.alt && (
          <img
            className={`${styles.FilterCustomImage} ${
              isAbsolute ? styles.ImageOn : styles.ImageOff
            }`}
            alt={props.alt}
            src={props.img}
          />
        ...
        <button
          ...
          aria-label={`${
            isOpen ? 'Abrir' : ' Fechar '
          } componentes de navegação entre os produtos`}
        >
          Filtrar
        ...
        <ul
          className={`${styles.FilterCustomWrapper} ${
            isOpen ? styles.FilterOpen : styles.FilterClose
          }`}
        >
          {props?.items?.map((element, index) => (
            <li key={index} className={styles.FilterCustomItem}>
              <a
                className={styles.FilterCustomItemLink}
                href={`#${element.id}`}
              >
