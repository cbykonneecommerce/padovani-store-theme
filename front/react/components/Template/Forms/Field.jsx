...
import { saveAttachment, saveData, validateForm } from '../../../utils/Forms'
...
const Field = ({
  ...
  const [current, setCurrent] = useState(
    send ? null : elem.checked != undefined ? elem.checked : null
  )
  ...
  const filterError = message?.error?.filter((find) => {
    if (find.name == elem.name) return find
  })
  ...
  useLayoutEffect(() => {
    ...
    if (elem.required) {
      ...
      if (filter.length == 0) {
        newRequired.push({
          name: elem.name,
          label: elem?.label ? elem?.label : elem.placeholder,
        })
    ...
    if (
      (elem.checked && data == null) ||
      (elem.checked && data[elem.name] == undefined && data[elem.name] == null)
    ) {
  ...
  const mask = (typed, type) => {
    if (type == 'date') {
      ...
      if (typed.length <= 9) return typed.replace(/(\d{5})(\d{3})/g, '$1-$2')
    } else if (type === 'cpf') {
      if (typed.length <= 11)
  ...
  return (
    <div
      className={`${styles.formGroup} ${styles[elem.name]} ${
        styles[elem.type]
      }`}
      style={
        elem.size && {
          width:
            elem.size == '100%' ? '100%' : `calc(${elem.size} - ${props.gap})`,
        }
      }
    >
      {props.showLabel &&
        type != 'title' &&
        type != 'submit' &&
        type != 'whatsapp' && <label>{elem.__editorItemTitle}</label>}
      ...
      {type == 'submit' && (
        <div>
          ...
          >
            {elem.icon && <img src={elem.icon} alt={elem.__editorItemTitle} />}
            {elem.__editorItemTitle}
          </button>
          <p style={{ color: 'red' }}>
            {errorMessageCustom ? 'Todos os campos devem ser preenchidos' : ''}
          </p>
        </div>
      )}
      {type == 'whatsapp' && (
        ...
        >
          {elem.icon && <img src={elem.icon} alt={elem?.label} />}
          {elem?.label}
        </button>
      )}
      {type == 'cep' && (
        <Input
          errorMessage={
            filterError?.length > 0
              ? `Campo ${
                  elem.label ? elem.label : elem.placeholder
                } é inválido!`
              : ''
          }
      ...
      {type == 'checkbox-description' && (
        <div className={styles.wrapperDescription}>
          ...
          />
          <label
            className={`vtex-checkbox__label ${styles.inputDescription}`}
      ...
      {type == 'address' ? (
        <div className={styles.address}>
          ...
          <div
            className={`${styles.formGroup} ${styles.zipcode}`}
            style={{ width: `calc(30% - ${props.gap})` }}
          >
            <label for="zipcode">CEP</label>
            <Input
              errorMessage={
                filterError?.length > 0 ? `Campo CEP é inválido!` : ''
              }
              error={filterError?.length > 0 ? true : false}
              ...
              value={zipCode}
              onChange={(e) => {
                findZipCode(e.currentTarget.value)
              }}
              maxLength="9"
            />
          </div>
          <div
            className={`${styles.formGroup} ${styles.street}`}
            style={{ width: `calc(70% - ${props.gap})` }}
          >
            <label for="street">Rua</label>
            <Input
              errorMessage={
                filterError?.length > 0 ? `Campo LOGRADOURO é inválido!` : ''
              }
              error={filterError?.length > 0 ? true : false}
          ...
          <div
            className={`${styles.formGroup} ${styles.number}`}
            style={{ width: `calc(33.3333% - ${props.gap})` }}
          >
            <label for="number">Número</label>
            <Input
              errorMessage={
                filterError?.length > 0 ? `Campo NÚMERO é inválido!` : ''
              }
              error={filterError?.length > 0 ? true : false}
          ...
          <div
            className={`${styles.formGroup} ${styles.complement}`}
            style={{ width: `calc(33.3333% - ${props.gap})` }}
          >
          ...
          <div
            className={`${styles.formGroup} ${styles.neighborhood}`}
            style={{ width: `calc(33.3333% - ${props.gap})` }}
          >
            <label for="neighborhood">Bairro</label>
            <Input
              errorMessage={
                filterError?.length > 0 ? `Campo BAIRRO é inválido!` : ''
              }
              error={filterError?.length > 0 ? true : false}
          ...
          <div
            className={`${styles.formGroup} ${styles.city}`}
            style={{ width: `calc(50% - ${props.gap})` }}
          >
            <label for="city">Cidade</label>
            <Input
              errorMessage={
                filterError?.length > 0 ? `Campo CIDADE é inválido!` : ''
              }
              error={filterError?.length > 0 ? true : false}
          ...
          <div
            className={`${styles.formGroup} ${styles.state}`}
            style={{ width: `calc(50% - ${props.gap})` }}
          >
            <label for="state">Estado</label>
            <Input
              errorMessage={
                filterError?.length > 0 ? `Campo ESTADO é inválido!` : ''
              }
              error={filterError?.length > 0 ? true : false}
      ...
      {type == 'file' && (
        <div className={styles.inputFile}>
          <label htmlFor="">
            IMAGENS INTERNAS/EXTERNAS DA LOJA. OS ARQUIVOS DAS IMAGENS DEVEM TER
            NO MÁXIMO 2M CADA:
          </label>
          <Dropzone
