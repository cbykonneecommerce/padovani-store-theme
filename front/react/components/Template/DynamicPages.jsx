    ...
    <div className={`${styles.DynamicPages}`}>
      {props?.sections &&
        props?.sections.map((item, index) => {
          return (
            <div
              className={`${styles[props.blockClass]} ${styles[item.layout]}`}
            >
              {props.separatorSVG && index > 0 && (
                <i
                  className={styles.icon}
                  dangerouslySetInnerHTML={{ __html: props.separatorSVG }}
                ></i>
              )}
              {item.imageDetail &&
              ...
              {item.layout != 'imagem' && (
                <div className={styles.showText}>
                  {item.svgIcon && item.layout != 'texto' && (
                    <i
                      className={styles.icon}
                      dangerouslySetInnerHTML={{ __html: item.svgIcon }}
                    ></i>
                  )}
                  {item.__editorItemTitle && item.layout != 'texto' && (
                    <h2 className={styles.heading}>{item.__editorItemTitle}</h2>
                  )}
                  {item.subtitle && item.layout != 'texto' && (
                    <h3 className={styles.subHeading}>{item.subtitle}</h3>
                  )}
                  {item.text && (
                  ...
                  {item.cta && item.layout != 'texto' && (
                    <a className={styles.cta}>{item.cta}</a>
                  )}
                </div>
              )}
                      ...
                      <div
                        ...
                        style={{
                          width: window.innerWidth
                            ? item.listWidthDesktop
                              ? item.listWidthDesktop
                              : '100%'
                            : item.listWidthMobile
                            ? item.listWidthMobile
                            : '100%',
                        }}
                      >
                        {list?.listImage[0]?.image && (
                          <img
                            className={styles.tagImg}
                            src={list?.listImage[0]?.image}
                            alt={list.__editorItemTitle}
                          />
                        )}
                        <div className={styles.textList}>
                          {list.svgIcon && (
                            <i
                              className={styles.icon}
                              dangerouslySetInnerHTML={{ __html: list.svgIcon }}
                            ></i>
                          )}
                          {list.__editorItemTitle && (
