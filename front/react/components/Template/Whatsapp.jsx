...
import styles from '../../styles/css/template.whatsapp.css';
...
const Whatsapp = ({...props}) => {
    ...
    useLayoutEffect(() => {
        ...
        if(link == null) {
            ...
            const productLink = `${window.location.origin}/${context?.product?.linkText}/p?=${context?.selectedItem?.itemId}`
            const reference = context?.product?.productReference
            const productName = context?.product?.productName

            const skuName = context?.selectedItem?.nameComplete

            const setMessage = "Olá! Pode me ajudar?"
            
            if(productLink)
                props.message.replaceAll("{NomeProduto}", productName).replaceAll("{LinkProduto}", productLink).replaceAll("{NomeSku}", skuName).replaceAll("{Referencia}", reference)
            
            setLink(`https://api.whatsapp.com/send?phone=${props.phone}&text=${setMessage}`)
...
Whatsapp.schema = {
    ...
    properties: {
        phone: {
            ...
            type: "string",
            description: "Ex.: 5511912349999"
        },
        message: {
            title: "Título",
            type: "string",
            description: "Chaves dinâmicas e não obrigatórias: {NomeProduto}, {LinkProduto}, {ImagemProduto}, {Referencia}, {NomeSku}"
        }
    }
}
