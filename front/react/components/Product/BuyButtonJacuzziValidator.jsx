...
const BuyButtonJacuzziValidator = (props) => {
  ...
  useEffect(() => {
    ...
    const findClusterFactory = context?.product?.productClusters?.find(
      (element) => {
        return element.id == '486' || element.id === '567'
      }
    )
    ...
    if (typeof document !== 'undefined') {
      const shippingSimulator = document.querySelector(
        '.vtex-flex-layout-0-x-flexRowContent--pdp-line-6'
      )

      if (findClusterFactory && shippingSimulator) {
        shippingSimulator.style.display = 'none'
      }
    }
  }, [context?.product])

  const findClusterFactory = context?.product?.productClusters?.some(
    (element) => element.id === '486' || element.id === '567'
  )
    ...
    <Skeleton height={70}>
      <div className={styles.BuyButtonJacuzziValidator}>
        {findClusterFactory ? (
          ...
          <div className={styles.BuyAreaWrapper}>{props?.children[0]}</div>
        )}
      </div>
    </Skeleton>
  )
...
BuyButtonJacuzziValidator.schema = {
  title: 'Editor Botão de Compra Da Fábrica',
  type: 'object',
  properties: {
    colorBg: {
      ...
      default: '#ebc072',
      description:
        'Você colocará a cor do fundo em Hexadecimal. Exemplo: #FF0000 = Cor vermelha',
    },
    colorkLetter: {
      ...
      default: '#704a00',
      description:
        'Você colocará a cor da letra em Hexadecimal. Exemplo: #FF0000 = Cor vermelha',
    },
    titleBtn: {
      title: 'Texto do botão',
      type: 'string',
      default: 'Comprar direto da Fábrica',
      description: 'Aqui você colocará o título desejado',
    },
    textBtn: {
      title: 'Texto do botão',
      type: 'string',
      default: 'fale com nossa equipe de vendas',
