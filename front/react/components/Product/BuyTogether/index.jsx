...
const BuyTogether = (props) => {
  ...
  async function getMainProduct() {
    if (context?.product?.items[0]) {
      setMainProduct({
        ...
        listPrice:
          context?.product?.items[0]?.sellers[0]?.commertialOffer?.ListPrice,
  ...
  return (
    <section ref={componentRef} className={styles.BuyTogetherContainer}>
            ...
            <ul className={styles.BuyTogetherInfoList}>
              {matchingCollection?.steps?.map((element, index) => (
                <li className={styles.BuyTogetherInfoItem}>
                  ...
                  <span className={styles.BuyTogetherInfoStep}>
                    {index + 1}
                  </span>
                  <span className={styles.BuyTogetherInfoStepText}>
                    {element.title}
                  </span>
                </li>
              ))}
      ...
      <BuyTogetherProvider
        products={products}
        baseColorCtx={matchingCollection?.baseColor}
      >
        ...
        {matchingCollection.thirdListIsActive &&
          (matchingCollection.componentType === 'colors' ? (
            ...
            <>
              {matchingCollection.thirdTitle && (
                <h2 className={styles.TileListProduct}>
                  {matchingCollection.thirdTitle}
                </h2>
              )}
        ...
        {matchingCollection.bombPositionIsActive && (
          <BombSelect
            ...
            images={context?.product?.items[0]?.images}
            title={matchingCollection?.title}
      ...
      items: {
        ...
        properties: {
            ...
            items: {
              ...
              properties: {
                ...
                iconDescription: {
                  ...
                  type: 'string',
                  description:
                    'Descreva a imagem. Ajuda na acessibilidade e SEO',
                  widget: {
                    'ui:widget': 'textarea',
                  },
                },
          ...
          bombPositionIsActive: {
            ...
            default: false,
            description:
              'Componente que o usuário seleciona posição da bomba em banheiras',
          },
        },
            ...
            oneOf: [
              {
                properties: {
                  ...
                  componentType: {
                    ...
                    type: 'string',
                    enum: ['colors', 'normal-products'],
                    enumNames: ['Cores', 'Produtos normais'],
                    default: 'normal-products',
                    description:
                      'Mostrar cores, para usuário navegar ou outros produtos para o usuário selecionar e comprar',
                  },
                },
